...
export default function StaffPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!staffRes.ok) throw new Error('Failed to get staff details');
  ...
  const toggleTheatreStatus = async (id: number) => {
    try {
      ...
      setTheatres((prev) =>
        prev.map((t) => (t.id === id ? { ...t, is_open: !t.is_open } : t))
      );
    } catch (err) {
      console.error(err);
    }
  ...
  const statusBadge = (
    ...
    return (
      <span className={`px-2 py-1 rounded-full font-medium ${colors[type]}`}>
        {text}
      </span>
    );
  };
      ...
      <div>
        ...
        <div>
          <p className="text-xl font-semibold mb-2 flex items-center gap-2">
            ...
            {currentStaff &&
              statusBadge(
                currentStaff.is_available ? 'Available' : 'Unavailable',
            ...
            <div className="space-y-2">
                      ...
                      if (data.user_id) {
                        setStaff((prev) => [
                          ...prev,
                          {
                            ...
                            name: newStaff.name,
                            role: newStaff.role,
                            theatre_id: newStaff.theatre_id,
                            is_available: true,
                          },
                        ]);
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewStaff((p) => ({
                          ...p,
                          role: e.target.value as 'admin' | 'runner',
                        }))
                      }
                      className="border rounded px-3 py-2 w-full"
                    >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewStaff((p) => ({
                          ...p,
                          theatre_id: Number(e.target.value),
                        }))
                      }
              ...
              <ul className="space-y-2">
                {staff.map((s) => (
                  ...
                  >
                    <span>
                      {s.name} ({s.role}) — Theatre {s.theatre_id}
                    </span>
                    <div className="flex items-center gap-2">
                      {statusBadge(
                        s.is_available ? 'Available' : 'Busy',
                        s.is_available ? 'green' : 'gray'
                      )}
                      <button
          ...
          <ul className="space-y-2">
            {deliveries.map((d) => {
              ...
              return (
                ...
                >
                  <span>
                    {d.productName} → {d.theatreName} (Qty: {d.quantity})
                  </span>
                  <div className="flex items-center gap-2">
                    {statusBadge(d.delivery_status, badgeColor)}
