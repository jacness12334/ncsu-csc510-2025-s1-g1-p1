...
export default function EditDetailsPage() {
  ...
  const handleAddPayment = async () => {
    ...
    let response = await fetch(
      'http://localhost:5000/api/customers/' + userId + '/payment-methods',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    const f = async () => {
      try {
        ...
        response = await fetch(
          'http://localhost:5000/api/customers/' + user_id,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        setPaymentMethods(
          rt.payment_methods.map((item: jsonPaymentMethodRecieveType) => {
            return {
              id: item.id,
