...
import uuid  # Generate unique identifiers to avoid FK/unique collisions in tests
...
MOCK_PASSWORD_HASH = "mock-test-hash"  # Matches Users.password_hash column
...
class TestDriverRoutes:
    # Create a driver (and user) with unique email/phone to prevent IntegrityError on reruns.
    def _create_test_driver(self, app, duty_status=STATUS_AVAILABLE, rating=5.0, deliveries=0, vehicle_color='Blue'):
        ...
        with app.app_context():  # DB ops require an application context
            user = Users(
                name='Test Driver',
                email=unique_email,
                phone=unique_phone,
            ...
            db.session.add(user)
            db.session.flush()  # Obtain user.id before creating dependent rows
    ...
    # Create a full test delivery
    def _create_test_delivery(self, app, driver_id=None, delivery_status='pending'):
        unique_id = uuid.uuid4().hex[:6]  # Avoid unique constraints across reruns

        with app.app_context():
            # Theatre (unique name+address per constraint)
            theatre = Theatres(
            ...
            # Staff (runner) and supplier (products depend on supplier)
            staff_user = Users(
                name='Test Staff',
                email=f'staff_{unique_id}@test.com',
                phone=f'555333{unique_id[:4]}',
            ...
            # Customer, showing booking
            customer_user = Users(
                name='Test Customer',
                email=f'customer_{unique_id}@test.com',
                phone=f'555555{unique_id[:4]}',
            ...
            # Product, cart item for the customer
            product = Products(
            ...
            cart_item = CartItems(customer_id=customer_user.id, product_id=product.id, quantity=2)
            ...
            # Delivery referencing the entire graph; optionally preset driver
            delivery = Deliveries(
                driver_id=driver_id,
                customer_showing_id=customer_showing.id,
                payment_method_id=payment_method.id,
                staff_id=staff.user_id,
                total_price=Decimal('20.00'),  # Matches 2 x 10.00 example
    ...
    # Parse a JSON error body safely; fall back to a readable string.
    ...
    def test_create_driver_success(self, client, app):
        # Creates a driver through the API and validates response shape.
        data = {
        ...
        assert response.status_code == 201  # Standard pytest assert style
        data = json.loads(response.data)
        assert data['message'] == 'Driver created successfully'
        assert 'user_id' in data

    def test_create_driver_missing_fields(self, client):
        # Omits required driver fields; should return a 400 with an error.
    ...
    def test_delete_driver_success(self, client, app):
        # Creates then deletes a driver; verifies 200 and subsequent 404 on GET.
        ...
        assert client.get(f'/api/driver/{user_id}').status_code == 404

    def test_delete_driver_not_found(self, client):
        # Deleting a non-existent driver should produce a 404 error.
    ...
    def test_get_driver_info_success(self, client, app):
        # Returns a driver info payload with expected values.
        user_id, _driver = self._create_test_driver(app, duty_status=STATUS_ON_DELIVERY, rating=4.9, deliveries=50)
    ...
    def test_get_driver_info_not_found(self, client):
        # Querying a missing driver should return 404.
    ...
    def test_update_driver_details_success(self, client, app):
        # Updates vehicle details and verifies persisted changes.
        ...
        verify = client.get(f'/api/driver/{user_id}')
        vdata = json.loads(verify.data)['driver']
        assert vdata['vehicle_color'] == 'Yellow'
        assert vdata['vehicle_type'] == 'bike'

    def test_update_driver_details_missing_fields(self, client, app):
        # Missing required fields should result in a 400 error.
        user_id, _ = self._create_test_driver(app)
        response = client.put(f'/api/driver/{user_id}', json={'vehicle_type': 'car'})
        assert response.status_code == 400
        data = json.loads(response.data)
        assert 'error' in data
    ...
    def test_update_driver_status_success(self, client, app):
        # Valid status update flips to available and returns confirmation.
        user_id, _ = self._create_test_driver(app, duty_status=STATUS_UNAVAILABLE)
        response = client.put(f'/api/driver/{user_id}/status', json={'new_status': STATUS_AVAILABLE})
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_update_driver_status_invalid_status(self, client, app):
        # Invalid enum value should be rejected with 400.
        user_id, _ = self._create_test_driver(app, duty_status=STATUS_UNAVAILABLE)
        response = client.put(f'/api/driver/{user_id}/status', json={'new_status': 'driving_around'})
        assert response.status_code == 400
        data = json.loads(response.data)
        assert 'error' in data

    def test_update_driver_status_missing_field(self, client, app):
        # Missing 'new_status' should produce a 400 error.
        user_id, _ = self._create_test_driver(app, duty_status=STATUS_UNAVAILABLE)
    ...
    def test_assign_driver_success(self, client, app):
        # Assigns the only available driver to a pending delivery.
    ...
    def test_assign_driver_no_available_drivers(self, client, app):
        # No available drivers should produce a 404 with a clear message.
    ...
    def test_complete_delivery_success(self, client, app):
        # Completes an assigned delivery and verifies the new status.
        ...
        if response.status_code != 200:  # Helpful failure message when debugging CI
    ...
    def test_complete_delivery_not_found(self, client):
        # Completing a non-existent delivery should return 400.
    ...
    def test_rate_driver_success(self, client, app):
        # Rates a fulfilled delivery and checks the updated average rating.
        driver_id, _ = self._create_test_driver(app, rating=4.0, deliveries=10)
        ...
        assert round(data['new_rating'], 2) == 4.09  # (4.0*10 + 5) / 11

    def test_rate_driver_missing_rating(self, client, app):
        # Missing rating field should produce a 400 error.
    ...
    def test_rate_driver_delivery_not_completed(self, client, app):
        # Only fulfilled deliveries can be rated; pending should fail with 400.
        driver_id, _ = self._create_test_driver(app)
    ...
    def test_get_active_delivery_found(self, client, app):
        # An in_progress delivery should be returned as the active delivery.
        driver_id, _ = self._create_test_driver(app)
    ...
    def test_show_completed_deliveries_found(self, client, app):
        # Returns only fulfilled deliveries in history.
        driver_id, _ = self._create_test_driver(app)
        ...
        self._create_test_delivery(app, driver_id=driver_id, delivery_status='accepted')  # Non-fulfilled
    ...
    def test_show_completed_deliveries_driver_not_found(self, client):
        # Unknown driver id should return 404 for history lookup.
