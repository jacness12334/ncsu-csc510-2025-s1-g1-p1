...
from app.models import (
    Users, Drivers, Deliveries, Theatres, Staff, CustomerShowings, PaymentMethods,
    CartItems, Products, Movies, Auditoriums, Seats, MovieShowings, Customers, Suppliers
)
...
# Mock hashed password for creating users
...
@pytest.fixture
def driver_service():
    # Provide a DriverService instance for tests
    return DriverService()

# ----------------------------
# Helper builders (DB objects)
# ----------------------------

def _create_test_driver(app, duty_status=STATUS_AVAILABLE, rating=Decimal('5.0'), deliveries=0, vehicle_color='Blue'):
    # Create a driver user and driver record with unique email/phone
...
def _create_delivery(app, driver_id, showing_id, payment_id, status, is_rated=False):
    # Create a delivery tied to a showing and payment method
    with app.app_context():
        delivery = Deliveries(
            driver_id=driver_id,
...
def setup_prerequisites(app):
    # Build theatre, movie/auditorium/seat/showing, customer/payment/showing, supplier/product/cart
    ...
    with app.app_context():
        ...
        # Movie and auditorium/seat/showing
        movie = Movies(
        ...
        # Supplier, product, and a cart item used for price calculations
        supplier_user = Users(
            name='Test Supplier', email=f'supplier_{unique_id}@test.com', phone=f'555444{unique_id[:4]}',
            birthday=datetime(1990, 1, 1), password_hash=MOCK_PASSWORD_HASH, role='supplier'
        )
        db.session.add(supplier_user)
        db.session.flush()
        supplier = Suppliers(
            user_id=supplier_user.id, company_name=f'Test Inc. {unique_id}',
            company_address='789 Supply', contact_phone='5557778888'
        )
        db.session.add(supplier)
        db.session.commit()

        product = Products(
            name=f'Soda {unique_id}', supplier_id=supplier.user_id, unit_price=Decimal('5.00'),
            inventory_quantity=10, category='beverages', size='small'
        )
...
class TestDriverService:
    # Expose the helper as a static method for reuse in tests
    ...
    def test_validate_rating_invalid(self, driver_service, rating, expected_error):
        # Ratings beyond bounds or non-numeric should raise
        with pytest.raises(ValueError, match=expected_error):
            driver_service.validate_rating(rating)

    def test_validate_rating_valid(self, driver_service):
        # Accept numeric ratings and return Decimal
    ...
    def test_create_driver_success(self, app, driver_service):
        # Create a driver and verify essential fields
        unique_id = uuid.uuid4().hex[:8]
        with app.app_context():
    ...
    def test_create_driver_invalid_role(self, driver_service):
        # Reject creation when role is not 'driver'
    ...
    def test_update_driver_details_success(self, app, driver_service):
        # Update vehicle fields and confirm values changed
    ...
    def test_update_driver_details_not_found(self, app, driver_service):
        # Reject updates for a missing driver
        with app.app_context():
    ...
    def test_update_driver_status_success(self, app, driver_service):
        # Change duty status and verify persisted value
    ...
    def test_delete_driver_success(self, app, driver_service):
        # Delete a driver and confirm user is removed
        user_id, driver = self._create_test_driver(app)
        with app.app_context():
    ...
    def test_get_available_drivers(self, app, driver_service):
        # One available and one unavailable; expect a single available result
    ...
    def test_get_best_available_driver(self, app, driver_service):
        # Pick the highest-rated available driver
    ...
    def test_try_assign_driver_success(self, app, driver_service, setup_prerequisites):
        # Assign an available driver to a pending delivery and verify changes
        driver_id, _ = self._create_test_driver(app, duty_status=STATUS_AVAILABLE)
        delivery_id = _create_delivery(
            app, driver_id=None,
            showing_id=setup_prerequisites['customer_showing_id'],
            payment_id=setup_prerequisites['payment_method_id'],
            status='pending'
        )
        with app.app_context():
    ...
    def test_try_assign_driver_no_available_driver(self, app, driver_service, setup_prerequisites):
        # No available drivers should leave delivery unassigned
    ...
    def test_try_assign_driver_delivery_not_found(self, driver_service):
        # Passing None should raise a not-found error (note: message spelling is defined by service)
    ...
    def test_complete_delivery_success(self, app, driver_service, setup_prerequisites):
        # Complete an accepted delivery and update driver stats/status
        ...
        with app.app_context():
            ...
            assert updated_driver.total_deliveries == 6

    def test_complete_delivery_not_found(self, app, driver_service):
        # Completing a non-existent delivery should raise
        with app.app_context():
    ...
    def test_complete_delivery_wrong_status(self, app, driver_service, setup_prerequisites):
        # Only accepted deliveries can be completed
    ...
    def test_rate_driver_success_new_average(self, app, driver_service, setup_prerequisites):
        # Apply a new rating and verify the recalculated average and delivery flag
        driver_id, driver = self._create_test_driver(app, rating=Decimal('4.00'), deliveries=10)
        delivery_id = _create_delivery(
            app, driver_id=driver_id,
            showing_id=setup_prerequisites['customer_showing_id'],
            payment_id=setup_prerequisites['payment_method_id'],
            status='fulfilled', is_rated=False
        )
        with app.app_context():
    ...
    def test_rate_driver_success_first_rating(self, app, driver_service, setup_prerequisites):
        # First rating should set the driver's rating directly
    ...
    def test_rate_driver_already_rated(self, app, driver_service, setup_prerequisites):
        # Re-rating a delivery should be rejected
        driver_id, driver = self._create_test_driver(app, rating=Decimal('4.00'), deliveries=10)
        delivery_id = _create_delivery(
            app, driver_id=driver_id,
            showing_id=setup_prerequisites['customer_showing_id'],
            payment_id=setup_prerequisites['payment_method_id'],
            status='fulfilled', is_rated=True
        )
        with app.app_context():
    ...
    def test_rate_driver_wrong_status(self, app, driver_service, setup_prerequisites):
        # Only fulfilled deliveries can be rated
        driver_id, driver = self._create_test_driver(app, rating=Decimal('4.00'), deliveries=10)
        delivery_id = _create_delivery(
            app, driver_id=driver_id,
            showing_id=setup_prerequisites['customer_showing_id'],
            payment_id=setup_prerequisites['payment_method_id'],
            status='delivered', is_rated=False
        )
        with app.app_context():
    ...
    def test_show_completed_deliveries_found(self, app, driver_service, setup_prerequisites):
        # Return only fulfilled deliveries and verify ids
        ...
        _create_delivery(app, driver_id, setup_prerequisites['customer_showing_id'], setup_prerequisites['payment_method_id'], 'pending')
        with app.app_context():
    ...
    def test_show_completed_deliveries_not_found(self, app, driver_service, setup_prerequisites):
        # If none are fulfilled, raise with a clear message
        driver_id, _ = self._create_test_driver(app)
        _create_delivery(app, driver_id, setup_prerequisites['customer_showing_id'], setup_prerequisites['payment_method_id'], 'accepted')
        with app.app_context():
    ...
    def test_get_active_delivery_found(self, app, driver_service, setup_prerequisites):
        # Retrieve the single active (in-progress/transit/accepted) delivery
        driver_id, _ = self._create_test_driver(app)
        _create_delivery(app, driver_id, setup_prerequisites['customer_showing_id'], setup_prerequisites['payment_method_id'], 'fulfilled')
        active_id = _create_delivery(app, driver_id, setup_prerequisites['customer_showing_id'], setup_prerequisites['payment_method_id'], 'in_transit')
        with app.app_context():
    ...
    def test_get_active_delivery_not_found(self, app, driver_service, setup_prerequisites):
        # When no active delivery exists, raise with a clear message
        driver_id, _ = self._create_test_driver(app)
        _create_delivery(app, driver_id, setup_prerequisites['customer_showing_id'], setup_prerequisites['payment_method_id'], 'fulfilled')
        with app.app_context():
